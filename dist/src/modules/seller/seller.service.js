...
const getSellerOverview = async (sellerId) => {
  ...
  FROM OrderStats;
`;
    const serializedResult = JSON.stringify(result, (key, value) => typeof value === "bigint" ? value.toString() : value);
    ...
    const chartOverview = {};
    const chartOverviewArray = [];
    console.log(getDatePart(recentOrders[0].createdAt.toISOString()));
    last30Days.forEach((day) => {
        const matchedOrders = recentOrders.filter((order) => getDatePart(order.updatedAt.toISOString()) == day);
        ...
        if (matchedOrders.length > 0) {
            ...
            matchedOrders.forEach((o) => {
                ...
                if (o.status != "pending" && o.status != "canceled") {
                    let count = 0;
                    o.items.forEach((i) => (count = i.productQuantity + count));
                    obj.items = count;
                }
            });
